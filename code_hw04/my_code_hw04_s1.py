...
def detect_planes(jparams):
    ...
    while(bool(remaining_pts)):
        ...
        while(bool(seed_stack)):
            ...
            while len(current_plane) <= 2:
                print("inside initial plane")
                central = seed_id
                nei = kd.query_ball_point(pts[central],jparams['epsilon'])
                print("nei",len(nei),nei)
                initial_nei =[]
                for p in nei:
                    if p in remaining_pts:
                        initial_nei.append(p)
                #print(initial_nei)
                if len(initial_nei) == 0:
                    #print("len(initial_nei) == 0",len(current_plane))
                    break
                elif len(initial_nei) >1 :
                    n = len(current_plane)
                    for p in initial_nei[0:3-n]:
                        seed_stack.append(p)
                    ...
                    remaining_pts.remove(p)
                    central = p
                #print("initial plane",len(current_plane),current_plane)
                #print("seed_stack",len(seed_stack),seed_stack)

            if len(current_plane) > 2:
